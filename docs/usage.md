...
You will need the path to a directory containing FastA files. This will import and attempt to perform SNP identification and create a phylogeny on all FastA files in the specified directory, as well as subdirectories.
...
- Valid file extenions: `fasta,fas,fna,fsa,fa` with optional gzip compression
...
| `file`   | Full path to FastA file. Files must have one of the following extensions ".fasta", ".fas", ".fna", ".fsa", ".fa" and can be optionally gzipped compressed. |
